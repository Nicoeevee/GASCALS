...
TSharedPtr<FSlateStyleSet> FGSCEditorStyle::StyleInstance = NULL;


FName FGSCEditorStyle::LaunchPadTitleKey = "GASCompanionEditor.LaunchPad.Tab.Title";
FName FGSCEditorStyle::LaunchPadPageTitleKey = "GASCompanionEditor.LaunchPad.Page.Title";
FName FGSCEditorStyle::LaunchPadItemTitleKey = "GASCompanionEditor.LaunchPad.Item.Title";
FName FGSCEditorStyle::LaunchPadItemSubTitleKey = "GASCompanionEditor.LaunchPad.Item.SubTitle";
...
TSharedRef<FSlateStyleSet> FGSCEditorStyle::Create()
{
	...
	Style->Set(LaunchPadTitleKey, DEFAULT_FONT("BoldCondensed", 24));
	Style->Set(LaunchPadPageTitleKey, DEFAULT_FONT("Bold", 16));
	Style->Set(LaunchPadItemTitleKey, DEFAULT_FONT("Bold", 12));
	Style->Set(LaunchPadItemSubTitleKey, DEFAULT_FONT("Light", 12));


	Style->Set("Icons.Discord", new IMAGE_BRUSH("icons/discord_icon_16x", Icon16x16));
	Style->Set("LaunchPad.Card.GASCompanionExamples", new IMAGE_BRUSH("LaunchPad/images/gsc_examples_plugin", Icon256x256));
	Style->Set("LaunchPad.Card.SamplePlayground", new IMAGE_BRUSH("LaunchPad/images/barbute", Icon256x256));
	Style->Set("LaunchPad.Card.SampleArchery", new IMAGE_BRUSH("LaunchPad/images/bowman", Icon256x256));
	Style->Set("LaunchPad.Card.SampleShooterCrosshair", new IMAGE_BRUSH("LaunchPad/images/crosshair", Icon256x256));
	Style->Set("LaunchPad.Card.SampleShooterFamas", new IMAGE_BRUSH("LaunchPad/images/famas", Icon256x256));
	Style->Set("LaunchPad.Card.SampleMagic", new IMAGE_BRUSH("LaunchPad/images/magic-swirl", Icon256x256));
	Style->Set("LaunchPad.Card.SampleAGR", new IMAGE_BRUSH("LaunchPad/images/run", Icon256x256));
	Style->Set("LaunchPad.Card.SampleALS", new IMAGE_BRUSH("LaunchPad/images/walk", Icon256x256));
	Style->Set("LaunchPad.Card.Rogue", new IMAGE_BRUSH("LaunchPad/images/rogue", Icon256x256));
	Style->Set("LaunchPad.Card.PowerButton", new IMAGE_BRUSH("LaunchPad/images/power-button", Icon256x256));
	Style->Set("LaunchPad.Card.Gladius", new IMAGE_BRUSH("LaunchPad/images/gladius", Icon256x256));

	// Common text styles
	{
		const FLinearColor Badass = FLinearColor(0.729412f, 0.854902f, 0.333333f, 1.f);
		const FLinearColor Tomato = FLinearColor(0.854902, 0.168741, 0.160237, 1.f);
	...
	// Common styles for launch pad actions
	{
		...
		const FLinearColor RoyalBlue = FLinearColor(0.254152f, 0.412543f, 0.887923f, 1.f);
		const FTextBlockStyle InheritedFromNativeTextStyle = FTextBlockStyle(NormalText)
			.SetFont(DEFAULT_FONT("Regular", 10))
			.SetColorAndOpacity(FSlateColor(RoyalBlue));

		Style->Set("LaunchPad.InheritedFromNativeTextStyle", InheritedFromNativeTextStyle);
		...
		Style->Set("LaunchPad.Action.Hyperlink", EditNativeHyperlinkStyle);
	}
