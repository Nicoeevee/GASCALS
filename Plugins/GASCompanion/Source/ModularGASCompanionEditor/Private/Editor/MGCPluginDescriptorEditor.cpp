...
#include "PluginBrowser/Private/PluginStyle.h"

#define LOCTEXT_NAMESPACE "PluginListTile"

void FMGCPluginDescriptorEditor::OpenEditorWindow(TSharedRef<IPlugin> Plugin, TSharedPtr<SWidget> ParentWidget, FSimpleDelegate OnEditCommitted)
{
	// Construct the plugin metadata object using the descriptor for this plugin
	UMGCPluginMetadaObject* MetadataObject = NewObject<UMGCPluginMetadaObject>();
	...
	PropertiesWindow->SetContent(
			...
			[
				...
				[
					...
					.OnClicked(FOnClicked::CreateStatic(&FMGCPluginDescriptorEditor::OnEditPluginFinished, MetadataObject, TSharedPtr<IPlugin>(Plugin), OnEditCommitted, TWeakPtr<SWindow>(PropertiesWindow)))
				]
			]
		);
...
FReply FMGCPluginDescriptorEditor::OnEditPluginFinished(UMGCPluginMetadaObject* MetadataObject, TSharedPtr<IPlugin> Plugin, FSimpleDelegate OnEditCommitted, TWeakPtr<SWindow> WeakWindow)
{
