...
public:
    ...
    /**
     * **Deprecated** Please don't use it in an attempt to bind abilities. Use the new MGCAbilitySystemComponent or Game Feature Action to bind abilities via Enhanced Input.
     *
     * Abilities will automatically activate when the input is pressed if InputID is not NONE.
     *
     * Input Action Mapping is required with a matching name to use these bindings.
     *
     * If you want more control and activate abilities from blueprint, set it to NONE and manually activate
     * the ability by class or tags from Player Character or Controller BP.
     */
	UE_DEPRECATED(4.27, "Deprecated, please don't use it in an attempt to bind abilities. Use the new MGCAbilitySystemComponent or Game Feature Action to bind abilities via Enhanced Input")
    // UPROPERTY(BlueprintReadOnly, EditAnywhere, Category = "GAS Companion|Ability")
    EGSCAbilityInputID InputID = EGSCAbilityInputID::None;
