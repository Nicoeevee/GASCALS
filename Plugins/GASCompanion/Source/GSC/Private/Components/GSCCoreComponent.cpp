...
void UGSCCoreComponent::SetupAbilityActor(AActor* InOwnerActor, AActor* InAvatarActor)
{
	SetupAbilityActor(OwnerAbilitySystemComponent, InOwnerActor, InAvatarActor);
}

void UGSCCoreComponent::SetupAbilityActor(UAbilitySystemComponent* ASC, AActor* InOwnerActor, AActor* InAvatarActor)
{
	if (!InOwnerActor)
	{
		GSC_LOG(Error, TEXT("UGSCCoreComponent::SetupAbilityActor() InOwnerActor is null"))
		return;
	}

	if (!InAvatarActor)
	{
		GSC_LOG(Error, TEXT("UGSCCoreComponent::SetupAbilityActor() InAvatarActor is null"))
		return;
	}
	...
	if (ASC)
	{
		...
		RegisterAbilitySystemDelegates(ASC);
	}
}
...
bool UGSCCoreComponent::ActivateAbilityByClass(const TSubclassOf<UGameplayAbility> AbilityClass, UGSCGameplayAbility*& ActivatedAbility, const bool bAllowRemoteActivation)
{
	...
	{
		GSC_LOG(Warning, TEXT("UGSCCoreComponent::ActivateAbilityByClass Couldn't get back active abilities with Class %s"), *AbilityClass->GetName());
	}
...
void UGSCCoreComponent::AddStartupAbilities()
{
	if (!OwnerActor)
	{
		return;
...
void UGSCCoreComponent::RemoveStartupAbilities()
{
	if (!OwnerActor)
	{
		return;
...
void UGSCCoreComponent::InitializeAttributes()
{
	if (!OwnerActor)
	{
		return;
	...
	{
		GSC_LOG(Error, TEXT("Missing DefaultAttributes for %s. Please fill in the pawn's Blueprint."), *GetName());
		return;
	}
...
void UGSCCoreComponent::AddStartupEffects()
{
	if (!OwnerActor)
	{
		return;
...
UGSCUWHud* UGSCCoreComponent::GetHUDWidget() const
{
	if (!OwnerActor)
	{
		return nullptr;
	...
	AGSCPlayerController* PC = Cast<AGSCPlayerController>(Character->GetController());
	if (!PC)
	{
		return nullptr;
	}

	return PC->UIHUDWidget;
}
