...
void UGSCComboManagerComponent::SetupOwner()
{
	...
	OwnerAbilitySystemComponent = UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(OwningCharacter);
	OwnerCoreComponent = UGSCBlueprintFunctionLibrary::GetCompanionCoreComponent(OwningCharacter);
}

UGameplayAbility* UGSCComboManagerComponent::GetCurrentActiveComboAbility()
{
	if (!OwnerAbilitySystemComponent)
	{
		GSC_LOG(Error, TEXT("UGSCCoreComponent::GetCurrentActiveComboAbility() ASC is not valid"))
		return nullptr;
	}
...
void UGSCComboManagerComponent::ActivateComboAbilityInternal(const TSubclassOf<UGSCGameplayAbility> AbilityClass, const bool bAllowRemoteActivation)
{
	...
	if (!OwnerCoreComponent)
	{
		GSC_LOG(Error, TEXT("UGSCComboManagerComponent::ActivateComboAbility() OwningCoreComponent is null"))
		return;
	}
